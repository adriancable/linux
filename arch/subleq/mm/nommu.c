...
/*
 * Subleq-optimized NOMMU memory management.
 *
 * This is a modified copy of mm/nommu.c with optimizations for Subleq's
 * slow instruction execution. Key changes:
 *
 * 1. do_mmap_private() uses power-of-2 compound page allocations instead
 *    of alloc_pages_exact() which splits pages individually.
 *
 * 2. free_page_series() detects compound pages via compound_order() and
 *    frees entire compound allocations at once, reducing free operations
 *    from O(n) to O(log n) for n pages.
 *
 * Example: 2560 pages (10MB) = 2048 + 512 pages = 2 frees instead of 2560.
 */
/*
 *  linux/mm/nommu.c
 *
 ...
 *
 * Subleq optimization: detect compound pages and free them by order.
 * This reduces the number of free operations from O(n) to O(log n)
 * when pages were allocated using power-of-2 compound allocations.
 */
static void free_page_series(unsigned long from, unsigned long to)
{
	while (from < to) {
		...
		unsigned long chunk_size = PAGE_SIZE << order;

		/*
		 * Safety check: don't free beyond 'to' boundary.
		 * This handles edge cases where the region doesn't
		 * align perfectly with compound page boundaries.
		 */
		if (from + chunk_size > to) {
			/* Fallback: free single pages for remainder */
		...
		atomic_long_sub(1 << order, &mmap_pages_allocated);
 ...
 *
 * Subleq optimization: use compound page allocations for efficient freeing.
 */
			   ...
			   unsigned long capabilities)
{
	 ...
	 * Round up to cover the full requested length.
	 */
	total_pages = (len + PAGE_SIZE - 1) >> PAGE_SHIFT;

	/*
	 * Try to allocate the largest power-of-2 that fits, then allocate
	 * the remainder. This is more memory-efficient than rounding up to
	 * the next power of 2.
	 *
	 * However, for simplicity and to guarantee contiguity, we first try
	 * a single high-order allocation. If that fails, we fall back to
...
alloc_success:
	atomic_long_add(total_pages, &mmap_pages_allocated);

	vm_flags_set(vma, VM_MAPPED_COPY);
	region->vm_flags = vma->vm_flags;
	region->vm_start = (unsigned long)base;
	region->vm_end   = region->vm_start + len;
	region->vm_top   = region->vm_start + (total_pages << PAGE_SHIFT);
